...
const QUICK_INITIAL = [
  ...
  { label: "⚙️ Internal tool",  val: "My team is using spreadsheets or a tool that doesn't fit — looking for something better" },
  { label: "💸 Pricing first",  val: "I want to understand what something like this would cost before going further" },
  { label: "🤷 Not sure yet",   val: "I know something needs to change in my business but I'm not sure what the solution is" },
];
...
/* ═══════════════════════════════════════════════════════════════
   PERSISTENCE — full conversation memory across sessions & page nav
═══════════════════════════════════════════════════════════════ */
const VKEY      = "gits_3d_v2";       // visitor profile
const CKEY      = "gits_conv_v2";     // conversation state
const GREETED_KEY = "gits_greeted";   // bubble shown flag — survives page nav
...
function loadVisitor(): VisitorData | null {
...
function loadConv(): ConvState | null {
  try {
    const raw = localStorage.getItem(CKEY);
    if (!raw) return null;
    const d: ConvState = JSON.parse(raw);
    // Expire after 7 days
    if (Date.now() - d.savedAt > 7 * 86400000) {
...
function saveConv(state: Partial<ConvState>) {
  try {
...
// Bubble shown flag — survives SPA navigation within same session
// Uses sessionStorage so it resets on true new tab/window open
...
function buildGITSRobot(THREE: any, scene: any) {
  const whiteMat = new THREE.MeshPhysicalMaterial({
    color: 0xF8F8F4, metalness: 0.10, roughness: 0.28, clearcoat: 0.85, clearcoatRoughness: 0.06,
  });
  const whiteLightMat = new THREE.MeshPhysicalMaterial({
    color: 0xFFFFFF, metalness: 0.06, roughness: 0.18, clearcoat: 0.95, clearcoatRoughness: 0.04,
  });
  const whiteOffMat = new THREE.MeshPhysicalMaterial({
    color: 0xE8E8E4, metalness: 0.08, roughness: 0.35, clearcoat: 0.50,
  });
  const veryDarkMat = new THREE.MeshPhysicalMaterial({
    color: 0x0C0C0B, metalness: 0.35, roughness: 0.55,
  });
  const darkPanelMat = new THREE.MeshPhysicalMaterial({
    color: 0x111110, metalness: 0.28, roughness: 0.62,
  });
  const visorMat = new THREE.MeshPhysicalMaterial({
    color: 0x080807, metalness: 0.08, roughness: 0.04, transparent: true, opacity: 0.95,
  });
  const eyeWhiteMat  = new THREE.MeshStandardMaterial({ color: 0xFFFFFF, roughness: 0.10, metalness: 0.0 });
  const eyePupilMat  = new THREE.MeshStandardMaterial({ color: 0x080807, roughness: 0.3 });
  const eyeShineMat  = new THREE.MeshStandardMaterial({ color: 0xFFFFFF, emissive: 0xFFFFFF, emissiveIntensity: 1.5 });
  const antGlowMat   = new THREE.MeshStandardMaterial({ color: 0xE8E8E4, emissive: 0xFFFFFF, emissiveIntensity: 0.9 });
  const earGlowMat   = new THREE.MeshStandardMaterial({ color: 0xFFFFFF, emissive: 0xFFFFFF, emissiveIntensity: 0.65 });
  const scanMat      = new THREE.MeshStandardMaterial({ color: 0xFFFFFF, emissive: 0xFFFFFF, emissiveIntensity: 0.45, transparent: true, opacity: 0.55 });
  const chestLight1Mat = new THREE.MeshStandardMaterial({ color: 0xFFFFFF, emissive: 0xFFFFFF, emissiveIntensity: 1.1 });
  const chestLight2Mat = new THREE.MeshStandardMaterial({ color: 0xDDDDDD, emissive: 0xFFFFFF, emissiveIntensity: 0.7 });
  const chestLight3Mat = new THREE.MeshStandardMaterial({ color: 0xBBBBBB, emissive: 0xFFFFFF, emissiveIntensity: 0.4 });
  const mouthMat     = new THREE.MeshStandardMaterial({ color: 0xFFFFFF, emissive: 0xFFFFFF, emissiveIntensity: 0.55, transparent: true, opacity: 0.88 });
  const shadowMat    = new THREE.MeshBasicMaterial({ color: 0x000000, transparent: true, opacity: 0.10, depthWrite: false });

  const M = (geo: any, mat: any, px = 0, py = 0, pz = 0, sx = 1, sy = 1, sz = 1, rx = 0, ry = 0, rz = 0) => {
  ...
  const HEAD_W = 2.72, HEAD_H = 2.18, HEAD_D = 2.10;

  const skull = new THREE.Mesh(new THREE.BoxGeometry(HEAD_W, HEAD_H, HEAD_D, 1, 1, 1), whiteLightMat);
  headGroup.add(skull);

  const topEdge = new THREE.Mesh(
    new THREE.BoxGeometry(HEAD_W - 0.04, 0.08, HEAD_D - 0.04),
    new THREE.MeshPhysicalMaterial({ color: 0xFFFFFF, metalness: 0.05, roughness: 0.10, clearcoat: 1.0 })
  );
  topEdge.position.y = HEAD_H / 2 + 0.04;
  headGroup.add(topEdge);

  [-HEAD_W / 2 - 0.01, HEAD_W / 2 + 0.01].forEach(() => {
    const sidePanel = new THREE.Mesh(new THREE.BoxGeometry(0.06, HEAD_H * 0.70, HEAD_D * 0.68), whiteOffMat);
    sidePanel.position.set(0, 0.05, 0);
    headGroup.add(sidePanel);
  });

  const VISOR_FRONT_Z = HEAD_D / 2;
  const visorHousing = new THREE.Mesh(new THREE.BoxGeometry(2.20, 1.38, 0.18), veryDarkMat);
  visorHousing.position.set(0, 0.08, VISOR_FRONT_Z - 0.01);
  headGroup.add(visorHousing);

  const visorGlass = new THREE.Mesh(new THREE.BoxGeometry(2.02, 1.22, 0.09), visorMat);
  visorGlass.position.set(0, 0.08, VISOR_FRONT_Z + 0.06);
  headGroup.add(visorGlass);

  const scanLine = new THREE.Mesh(new THREE.BoxGeometry(1.94, 0.055, 0.06), scanMat);
  scanLine.position.set(0, 0.08, VISOR_FRONT_Z + 0.10);
  headGroup.add(scanLine);

  const EYE_X_OFFSET = 0.52, EYE_Y = 0.22, EYE_Z = VISOR_FRONT_Z + 0.12, EYE_RADIUS = 0.255;

  function makeEye(xPos: number) {
    const g = new THREE.Group();
    g.position.set(xPos, EYE_Y, EYE_Z);
    const white  = new THREE.Mesh(new THREE.SphereGeometry(EYE_RADIUS, 24, 18), eyeWhiteMat);
    g.add(white);
    const pupil = new THREE.Mesh(new THREE.CylinderGeometry(0.115, 0.115, 0.035, 20), eyePupilMat);
    pupil.rotation.x = Math.PI / 2;
    pupil.position.set(0, 0, EYE_RADIUS - 0.01);
    g.add(pupil);
    const shine = new THREE.Mesh(new THREE.SphereGeometry(0.048, 10, 8), eyeShineMat);
  ...
  const mouth = new THREE.Mesh(new THREE.BoxGeometry(0.98, 0.095, 0.07), mouthMat);
  ...
  antLGroup.add(M(new THREE.SphereGeometry(0.075, 10, 8), veryDarkMat));
  ...
  antRGroup.add(M(new THREE.SphereGeometry(0.092, 10, 8), veryDarkMat));
  ...
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.38, 0.50, 0.58, 14), whiteMat);
  neck.position.y = 2.56;
  root.add(neck);

  const torso = new THREE.Mesh(new THREE.BoxGeometry(2.32, 2.18, 1.50), whiteMat);
  ...
  const chestPanel = new THREE.Mesh(new THREE.BoxGeometry(1.72, 1.22, 0.10), darkPanelMat);
  chestPanel.position.set(0, 1.30, 0.82);
  root.add(chestPanel);

  const gitsLabel = new THREE.Mesh(new THREE.BoxGeometry(1.14, 0.32, 0.08), veryDarkMat);
  ...
  const backPanel = new THREE.Mesh(new THREE.BoxGeometry(1.80, 1.42, 0.09), darkPanelMat);
  backPanel.position.set(0, 1.24, -0.82);
  root.add(backPanel);

  for (let i = 0; i < 5; i++) {
    const vent = new THREE.Mesh(new THREE.BoxGeometry(1.38, 0.058, 0.10), veryDarkMat);
  ...
  const hip = new THREE.Mesh(new THREE.BoxGeometry(1.94, 0.48, 1.38), whiteOffMat);
  hip.position.y = 0.16;
  root.add(hip);

  const hipPanel = new THREE.Mesh(new THREE.BoxGeometry(1.54, 0.28, 0.08), darkPanelMat);
  ...
  return {
    root, headGroup,
    scanLine, scanMat,
    eyeGroupL: eyeL.group, eyeGroupR: eyeR.group,
    pupilL: eyeL.pupil, pupilR: eyeR.pupil,
    shineL: eyeL.shine, shineR: eyeR.shine,
    EYE_RADIUS, EYE_X_OFFSET, EYE_Y, EYE_Z,
    mouth, mouthMat,
...
export default function GITSAdvisor3D() {
  ...
  /* ── Stable session ID + restore conversation on mount ── */
  useEffect(() => {
    ...
    // Restore saved conversation if it exists
    const conv = loadConv();
  ...
  useEffect(() => {
    ...
    import("three").then((THREE) => {
      ...
      function computePupilOffset(eyeGroup: any, ndcX: number, ndcY: number, eyeRadius: number) {
        ...
        if (dist > maxOff) { const s = maxOff / dist; px *= s; py *= s; }
        return { px, py };
      }
      ...
      function loop() {
        ...
        { const s = spring(sp.posY.v, anim.targetY, sp.posY.vel, 0.036, 0.75); sp.posY.v = s.value; sp.posY.vel = s.velocity; r.root.position.y = sp.posY.v; }
        const risen = sp.posY.v > -2.5;
        if (risen) { r.root.position.y += Math.sin(t * 1.74) * 0.018 + Math.sin(t * 3.1) * 0.007; }
        const patrolTarget = Math.sin(t * 0.20) * 0.62;
        { const s = spring(sp.rotY.v, patrolTarget, sp.rotY.vel, 0.020, 0.77); sp.rotY.v = s.value; sp.rotY.vel = s.velocity; r.root.rotation.y = sp.rotY.v; }
        { const s = spring(sp.headRotY.v, anim.ndcX * 0.28, sp.headRotY.vel, 0.058, 0.77); sp.headRotY.v = s.value; sp.headRotY.vel = s.velocity; r.headGroup.rotation.y = sp.headRotY.v; }
        { const s = spring(sp.headRotX.v, -anim.ndcY * 0.14, sp.headRotX.vel, 0.058, 0.77); sp.headRotX.v = s.value; sp.headRotX.vel = s.velocity; r.headGroup.rotation.x = sp.headRotX.v; }
        { const bobTarget = Math.sin(t * 1.74 + 0.4) * 0.020; const s = spring(sp.headY.v, bobTarget, sp.headY.vel, 0.11, 0.73); sp.headY.v = s.value; sp.headY.vel = s.velocity; r.headGroup.position.y = 3.50 + sp.headY.v; }
        ...
        { const sx = spring(sp.pupilLX.v, tL.px, sp.pupilLX.vel, 0.14, 0.72); sp.pupilLX.v = sx.value; sp.pupilLX.vel = sx.velocity; }
        { const sy = spring(sp.pupilLY.v, tL.py, sp.pupilLY.vel, 0.14, 0.72); sp.pupilLY.v = sy.value; sp.pupilLY.vel = sy.velocity; }
        { const sx = spring(sp.pupilRX.v, tR.px, sp.pupilRX.vel, 0.14, 0.72); sp.pupilRX.v = sx.value; sp.pupilRX.vel = sx.velocity; }
        { const sy = spring(sp.pupilRY.v, tR.py, sp.pupilRY.vel, 0.14, 0.72); sp.pupilRY.v = sy.value; sp.pupilRY.vel = sy.velocity; }

        const R = r.EYE_RADIUS;
        function applyPupilOnSphere(pupil: any, shine: any, ox: number, oy: number) {
          const maxR = R * 0.54;
          ...
          const nx = fx / R, ny = fy / R;
          const angle = Math.acos(Math.max(-1, Math.min(1, pz / R)));
          if (angle > 0.001) {
            const ax = -ny, ay = nx;
            const len = Math.sqrt(ax * ax + ay * ay);
            if (len > 0.001) { pupil.setRotationFromAxisAngle(new THREE.Vector3(ax / len, ay / len, 0), angle); }
          }
          shine.position.set(fx + 0.062, fy + 0.062, pz + 0.015);
        }
        ...
        { const s = spring(sp.antLY.v, Math.sin(t * 1.30) * 0.12, sp.antLY.vel, 0.08, 0.74); sp.antLY.v = s.value; sp.antLY.vel = s.velocity; r.antLGroup.rotation.z = sp.antLY.v - 0.06; }
        { const s = spring(sp.antRY.v, Math.sin(t * 1.10 + 0.4) * 0.10, sp.antRY.vel, 0.08, 0.74); sp.antRY.v = s.value; sp.antRY.vel = s.velocity; r.antRGroup.rotation.z = sp.antRY.v + 0.05; }
        ...
        if (m === "talking") { r.mouth.scale.x = 0.85 + 0.35 * Math.abs(Math.sin(t * 9.2)); }
        else { r.mouth.scale.x += (1.0 - r.mouth.scale.x) * 0.08; }

        let tArmLZ =  Math.PI / 16, tArmRZ = -Math.PI / 16, tArmLX = 0.05, tArmRX = 0.05;
        if (m === "idle" || m === "curious") {
          ...
          tArmRZ = -Math.PI / 16 - 0.64; tArmRX = 0.38;
          tArmLZ =  Math.PI / 16 + Math.sin(t * 1.0) * 0.042;
        } else if (m === "excited") {
          tArmLZ =  Math.PI / 16 + Math.sin(t * 7.5) * 0.95; tArmLX = -0.28; tArmRZ = -Math.PI / 16 - 0.28;
        } else if (m === "handsup") {
          tArmLZ = -Math.PI * 0.85; tArmRZ =  Math.PI * 0.85;
        }
        { const s = spring(sp.armLZ.v, tArmLZ, sp.armLZ.vel, 0.088, 0.74); sp.armLZ.v = s.value; sp.armLZ.vel = s.velocity; r.armL.rotation.z = sp.armLZ.v; }
        { const s = spring(sp.armRZ.v, tArmRZ, sp.armRZ.vel, 0.088, 0.74); sp.armRZ.v = s.value; sp.armRZ.vel = s.velocity; r.armR.rotation.z = sp.armRZ.v; }
        { const s = spring(sp.armLX.v, tArmLX, sp.armLX.vel, 0.095, 0.75); sp.armLX.v = s.value; sp.armLX.vel = s.velocity; r.armL.rotation.x = sp.armLX.v; }
        { const s = spring(sp.armRX.v, tArmRX, sp.armRX.vel, 0.095, 0.75); sp.armRX.v = s.value; sp.armRX.vel = s.velocity; r.armR.rotation.x = sp.armRX.v; }
        if (m === "excited") { r.root.position.y += Math.sin(t * 11.5) * 0.11; }
      ...
      function onScroll() {
        ...
        // Only show bubble once per browser session — never re-trigger on page nav
        if (anim.scrollCount === 3 && !hasGreetedThisSession()) {
          markGreetedThisSession();
          setTimeout(() => {
            const v    = loadVisitor();
            const conv = loadConv();
            const hasHistory = conv && conv.messages.length > 1;

            let txt: string;
            if (hasHistory && conv.summary) {
              // They had a real conversation — pick up where they left off
              const topic = conv.summary.slice(0, 120);
              txt = `👋 Welcome back!\n\nLast time we were talking about **${topic}**\n\nWant to pick up where we left off?`;
            } else if (hasHistory) {
            ...
            setTimeout(() => {
              if (threeRef.current) { threeRef.current.anim.mode = "idle"; setRobotMood("idle"); }
            }, 1400);
          }, 800);
        }
      }
      ...
      const riseTimer = setTimeout(() => { if (anim.targetY < 0) anim.targetY = 0; }, 2600);

      function onMouse(e: MouseEvent) {
        if (!canvas) return;
        const rect = canvas.getBoundingClientRect();
        anim.ndcX = Math.max(-1, Math.min(1,  ((e.clientX - rect.left)  / rect.width)  * 2 - 1));
        anim.ndcY = Math.max(-1, Math.min(1, -(((e.clientY - rect.top)   / rect.height) * 2 - 1)));
      }
      ...
      function onEnter() { if (threeRef.current?.anim.mode === "idle") { threeRef.current.anim.mode = "curious"; setRobotMood("curious"); } }
      function onLeave() { if (threeRef.current?.anim.mode === "curious") { threeRef.current.anim.mode = "idle"; setRobotMood("idle"); } }
      if (canvas) { canvas.addEventListener("mouseenter", onEnter); canvas.addEventListener("mouseleave", onLeave); }
      ...
      cleanupFn = () => {
        ...
        if (canvas) { canvas.removeEventListener("mouseenter", onEnter); canvas.removeEventListener("mouseleave", onLeave); }
  ...
  const setRobotMode = useCallback((mode: string, ms = 0) => {
    ...
    setRobotMood(mode);
    if (ms > 0) setTimeout(() => {
      if (threeRef.current?.anim) { threeRef.current.anim.mode = "idle"; setRobotMood("idle"); }
    }, ms);
  }, []);
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    const assistantMsg = { role: "assistant", content: reply, time: Date.now() };
      ...
      if (hasHistory) {
        ...
        setTimeout(() => {
          ...
          const summary = conv.summary
            ? `Last time we were talking about **${conv.summary.slice(0, 100)}**. `
            : "";
          const followUp = {
            role: "assistant",
            content: `${summary}Want to pick up where we left off, or is there something new on your mind?`,
            time: Date.now(),
          };
          setMessages(prev => [...prev, followUp]);
          historyRef.current.push({ role: "assistant", content: followUp.content });
          setQuickReplies([
            { label: "↩ Continue",       val: "Yes, let's continue where we left off" },
            { label: "🆕 New topic",      val: "Actually I have something different in mind" },
            { label: "💰 Rough budget",   val: "I just want to know rough pricing" },
            { label: "👋 Talk to someone", val: "I'd like to speak with someone directly" },
          ]);
          setUnread(0);
          isAtBottomRef.current = true;
      ...
      <div id="g3d-root" data-chat={chatOpen ? "1" : "0"}>
        ...
        {bubble.show && !chatOpen && (
          <div className="g3d-bubble">
            ...
            <div className="g3d-bubble-ctas">
              <button className="g3d-bubble-yes" onClick={() => { setBubble(b => ({ ...b, show: false })); fireCTAPush("bubble_cta"); openChat(); }}>
                Let's talk →
              </button>
        ...
        <div className={`g3d-chat${chatOpen ? " open" : ""}`} role="dialog" aria-label="GITS AI Advisor" aria-modal="true">
          ...
          <div className="g3d-head">
            ...
            <button className="g3d-icon-btn g3d-clear-btn" onClick={() => { setConfirmClear(c => !c); setMenuOpen(false); }} aria-label="Clear conversation" title="Clear chat">
          ...
          <div
            ...
            onClick={(e) => {
              ...
              const ctaMap: Record<string, string> = { "wa.me": "whatsapp", "calendly.com": "book_call", "/contact": "contact", "/audit": "audit" };
              for (const [key, ctaType] of Object.entries(ctaMap)) {
                if (href.includes(key)) { fireCTAPush(ctaType); break; }
              }
            }}
          >
            {messages.map((msg, i) => (
          ...
          {showScrollBtn && (
            <button className={`g3d-scroll-btn${newMsgWhileUp > 0 ? " has-new" : ""}`} onClick={scrollToBottom} aria-label="Scroll to latest message">
              {newMsgWhileUp > 0 && <span className="g3d-new-badge">{newMsgWhileUp} new</span>}
          ...
          <div className="g3d-input-row">
            <textarea ref={textareaRef} className="g3d-input" value={inputVal} rows={1}
              ...
              onKeyDown={e => {
                if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); if (inputVal.trim() && !isTyping) sendMessage(inputVal.trim()); }
              }}
            />
            <button className={`g3d-send${isTyping ? " sending" : ""}`} disabled={!inputVal.trim() || isTyping} onClick={() => { if (inputVal.trim() && !isTyping) sendMessage(inputVal.trim()); }} aria-label="Send">
              {isTyping ? (
...
/* ═══════════════════════════════════════════════════════════════
   CSS — identical to v2, preserved in full
═══════════════════════════════════════════════════════════════ */
const CSS = `
:root {
  --gb:   #080807;
...
.g3d-canvas {
  ...
  filter: drop-shadow(0 8px 28px rgba(0,0,0,0.45)) drop-shadow(0 2px 8px rgba(0,0,0,0.38));
  transition: filter 0.35s ease;
}
.g3d-canvas:hover { filter: drop-shadow(0 14px 44px rgba(0,0,0,0.55)) drop-shadow(0 2px 12px rgba(0,0,0,0.45)); }
.g3d-canvas:focus-visible { outline: 2px solid var(--gbr3); outline-offset: 4px; border-radius: 10px; }
.g3d-badge {
  position: absolute;
  bottom: 186px;
  right: -3px;
  ...
  animation: g3d-badge-in 0.30s cubic-bezier(0.34,1.6,0.64,1), g3d-badge-pulse 2.2s ease-in-out infinite 0.6s;
}
...
.g3d-head { display:flex; align-items:center; gap:6px; padding:10px; border-bottom:1px solid var(--gbr); background:var(--gb3); flex-shrink:0; position:relative; }
...
.g3d-clear-btn { color:var(--gw3) !important; transition:color .2s,background .2s,transform .15s !important; }
.g3d-clear-btn:hover { color:var(--gdanger) !important; background:rgba(224,82,82,0.12) !important; transform:scale(1.05); }
...
.g3d-close { display:flex; align-items:center; justify-content:center; flex-shrink:0; width:28px; height:28px; background:var(--gb4); border:1px solid var(--gbr2); border-radius:8px; cursor:pointer; color:var(--gw2); transition:background .18s,color .18s,transform .15s; padding:0; }
.g3d-close:hover { background:var(--gb5); color:var(--gw); transform:scale(1.08); }
.g3d-confirm-strip { display:flex; align-items:center; justify-content:space-between; gap:8px; padding:9px 14px; background:rgba(224,82,82,0.08); border-bottom:1px solid rgba(224,82,82,0.18); animation:g3d-strip-in .22s ease both; flex-shrink:0; }
...
.g3d-msgs { flex:1; overflow-y:auto; padding:14px 12px 6px; display:flex; flex-direction:column; gap:10px; scroll-behavior:smooth; min-height:80px; scrollbar-width:thin; scrollbar-color:var(--gb4) transparent; }
...
.g3d-scroll-btn { position:absolute; bottom:90px; left:50%; transform:translateX(-50%); display:flex; align-items:center; gap:5px; background:var(--gb3); border:1px solid var(--gbr2); color:var(--gw2); font-family:var(--gf); font-size:11px; font-weight:600; padding:6px 14px 6px 10px; border-radius:100px; cursor:pointer; box-shadow:var(--gsh2); animation:g3d-scroll-in .28s cubic-bezier(0.34,1.6,0.64,1); transition:background .18s,color .18s,transform .15s; z-index:10; white-space:nowrap; }
...
.g3d-quicks-inner { display:flex; gap:5px; overflow-x:auto; padding:0 12px; scrollbar-width:none; -ms-overflow-style:none; }
...
.g3d-footer { display:flex; align-items:center; justify-content:space-between; font-family:var(--gm); font-size:8px; color:var(--gw4); letter-spacing:.08em; padding:5px 12px 9px; background:var(--gb3); flex-shrink:0; }
...
@media(max-width:440px){
  ...
  .g3d-chat { width:calc(100vw - 24px)!important; right:12px!important; bottom:180px!important; max-height:calc(100vh - 180px - 70px)!important; border-radius:18px; }
  .g3d-bubble { width:214px; right:0; }
  .g3d-badge { bottom:158px; }
  .g3d-scroll-btn { bottom:100px; }
}
@media(max-width:360px){
  .g3d-chat { width:calc(100vw - 16px)!important; right:8px!important; }
}
@media(prefers-reduced-motion:reduce){
  .g3d-bubble,.g3d-chat,.g3d-q,.g3d-row,.g3d-dots span,.g3d-badge,.g3d-confirm-strip,.g3d-scroll-btn { animation:none!important; transition:none!important; }
