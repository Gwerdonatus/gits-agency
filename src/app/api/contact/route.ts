...
export async function POST(req: Request) {
  try {
    ...
    // Honeypot
    const company = clean((body as any).company, 120);
    if (company) return NextResponse.json({ ok: true });

    const name = clean((body as any).name, 120);
    const email = clean((body as any).email, 200);
    const projectType = clean((body as any).projectType, 80);
    const timeline = clean((body as any).timeline, 80);
    const budget = clean((body as any).budget, 80);
    const prefer = clean((body as any).prefer, 80);
    const phone = clean((body as any).phone, 80);
    const message = clean((body as any).message, 6000);

    const okEmail = /^\S+@\S+\.\S+$/.test(email);

    if (name.length < 2) return NextResponse.json({ ok: false, error: "Name is required." }, { status: 400 });
    if (!okEmail) return NextResponse.json({ ok: false, error: "Valid email is required." }, { status: 400 });
    if (message.length < 10) return NextResponse.json({ ok: false, error: "Message is too short." }, { status: 400 });
