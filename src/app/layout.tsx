...
// ─── CONDITIONAL WRAPPER ─────────────────────────────────────────────────────
// Hides universal Navbar/Footer on /elowen-living/* routes so the page
// uses its own dedicated layout.tsx nav and footer instead.
// ──────────────────────────────────────────────────────────────────────────────
import { headers } from "next/headers";

async function ConditionalNavFooter({ children }: { children: React.ReactNode }) {
  const h = await headers();
  const pathname = h.get("x-pathname") ?? "";
  const isElowen = pathname.startsWith("/elowen-living");
